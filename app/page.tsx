  ...
  return (
    <main className="min-h-screen bg-navy-deep text-cream">
      ...
      <div className="mx-auto max-w-4xl px-4 py-14 md:px-6">
        <h1 className="font-serif text-3xl font-bold md:text-4xl">Términos y condiciones</h1>
        <p className="mt-3 text-cream/70">
          Última actualización: {new Date().toLocaleDateString("es-CL", { year: "numeric", month: "long", day: "numeric" })}
        </p>
        <p className="mt-6 text-sm text-cream/70 leading-relaxed max-w-2xl">
        ...
        <div className="mt-10 space-y-4">
          {terminos.map((item, idx) => (
            <div
              key={idx}
              className="rounded-2xl border border-cream/10 bg-navy/60 px-6 py-5 shadow-lg shadow-black/40"
            >
              <h2 className="font-semibold text-cream">{item.title}</h2>
              <p className="mt-2 text-sm leading-relaxed text-cream/75">{item.content}</p>
            </div>
          ))}
        </div>

        <div className="mt-12 rounded-2xl border border-celeste/20 bg-navy/60 p-6 text-sm text-cream/70">
          ¿Tienes dudas sobre estos términos? Escríbenos por WhatsApp al{" "}
          <a
            href="https://wa.me/56991513393"
            target="_blank"
            className="text-celeste underline decoration-celeste/50 underline-offset-4"
          >
            +56 9 9151 3393
          </a>{" "}
          y te ayudamos a aclararlas.
        </div>
